...
func main() {
	...
	fmt.Println(totalCost([]int{10, 1, 11, 10}, 2, 1))
}
...
func totalCost(costs []int, k int, candidates int) int64 {
	...
	for i := 0; i < candidates && left <= right; i++ {
		heap.Push(hleft, costs[left])
		left++
	}
	for i := 0; i < candidates && left <= right; i++ {
		heap.Push(hright, costs[right])
		right--
	}
	...
	for range k {
		...
		if hleft.Len() == 0 {
			p = heap.Pop(hright).(int)
		} else if hright.Len() == 0 {
			p = heap.Pop(hleft).(int)
